...
public class PasswordAuditAdvice {
	...
	/**
	 * Resolve the audit event type to log based on the invoked method, the current password reset flow
	 * state for the session, and whether it succeeded.
	 *
	...
	private AuditSecurityEventType resolveEventType(String methodName, String sessionId, boolean success) {
		...
		if ("changePassword".equals(methodName)) {
			if (PasswordResetFlowContext.hasPendingResetRequest(sessionId)) {
				return success ? AuditSecurityEventType.PASSWORD_RESET_SUCCESS
				        : AuditSecurityEventType.PASSWORD_RESET_FAILURE;
			}
			return success ? AuditSecurityEventType.PASSWORD_CHANGED_SUCCESS
			        : AuditSecurityEventType.PASSWORD_CHANGED_FAILURE;
		}
		
		return success ? AuditSecurityEventType.PASSWORD_RESET_REQUEST_SUCCESS
