...
/**
 * Utility class providing helper methods related to Hibernate Envers auditing and system user
 * checks within the OpenMRS runtime context.
 */
public class EnversUtils {
	
	/**
	 * Checks whether Hibernate Envers auditing is enabled based on the runtime property
	 * {@code hibernate.integration.envers.enabled}.
	 *
	...
	/**
	 * Determines whether the currently authenticated user has the 'System Developer' role, indicating
	...
	/**
	 * Builds a Hibernate Envers {@link AuditQuery} for a given entity class, applying optional filters
	 * by user and date range, and paginating the results.
	 *
	 ...
	 */
	public static <T> AuditQuery buildFilteredAuditQuery(AuditReader auditReader, Class<T> entityClass, Integer userId,
	        Date startDate, Date endDate, int page, int size, String sortOrder) {
	...
	/**
	 * Builds a Hibernate Envers count query with optional filters for user and date range. This is
	 * typically used to count how many filtered revisions exist for pagination.
	 *
	 * @param auditReader the {@link AuditReader} instance for querying Envers data
	 * @param entityClass the audited entity class
	 ...
	 */
	public static <T> AuditQuery buildCountQueryWithFilters(AuditReader auditReader, Class<T> entityClass, Integer userId,
	...
	/**
	 * Applies common filters (user ID and date range) to a given {@link AuditQuery}. This method is
	 * reused by both query-building methods.
	 *
