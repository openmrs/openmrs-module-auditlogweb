...
public class HibernateReadAuditDao implements ReadAuditDAO {
    ...
    @Override
    public List<ReadAuditLog> getReadAuditLogs(String entityType, String username, Date startDate, Date endDate, int page, int size) {
        String hql = buildQueryConditions("from ReadAuditLog e where 1=1", entityType, username, startDate, endDate)
                + " order by e.eventTime desc";

        Query<ReadAuditLog> query = sessionFactory.getCurrentSession()
                .createQuery(hql, ReadAuditLog.class);
    ...
    public long countReadAuditLogs(String entityType, String username, Date startDate, Date endDate) {
        String hql = buildQueryConditions("select count(e) from ReadAuditLog e where 1=1", entityType, username, startDate, endDate);
    ...
    public List<ReadAuditLog> getRelatedReadLogs(String sessionId, int limit) {
        Query<ReadAuditLog> query = sessionFactory.getCurrentSession()
                .createQuery("from ReadAuditLog e where e.sessionId = :sessionId order by e.eventTime desc",
                        ReadAuditLog.class);
